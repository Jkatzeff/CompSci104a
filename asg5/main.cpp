...
void createSymFile(const char* filename){
   string symOut=basename(const_cast<char*>(filename));
   size_t last= symOut.find_last_of('.'); 
   symOut.erase(last+1,2);
   symOut.append("sym");
   sym_file=fopen(symOut.c_str(), "w");
   if(sym_file==NULL)
   {
      fprintf(stderr, "Invalid file open (sym)");
      exit(EXIT_FAILURE);
   }
}

void createOilFile(const char* filename){
   string oilOut=basename(const_cast<char*>(filename));
   size_t last= oilOut.find_last_of('.'); 
   oilOut.erase(last+1,2);
   oilOut.append("oil");
   oil_file=fopen(oilOut.c_str(), "w");
   if(oil_file==NULL)
   {
      fprintf(stderr, "Invalid file open (oil)");
      exit(EXIT_FAILURE);
   }
}
...
int main(int argc, char** argv)
{
   ...
   createSymFile(filename);
   createOilFile(filename);
   yyparse();
   cpp_pclose();
   makeStringSet(filename);
   trav_attr(yyparse_astree);
   traversal(yyparse_astree);
   makeASTree(filename);
   emit_oil(oil_file, yyparse_astree);
   fclose(token_file);
   fclose(sym_file);
   fclose(oil_file);
